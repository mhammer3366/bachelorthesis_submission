...
# xlsr_300m_layer8.py
# Multi-GPU (DataParallel) XLS-R 300M layer-8 feature extraction, using label_id.
...
out_dir = str(REPO_ROOT / "data_preparation/feature_extraction/ohne_deutsch/xlsr_300m_layer8")
...
# tune these
batch_size = 64            # XLS-R 300M is larger, so smaller batch size
sample_rate = 16000
max_length = 80000         # ~5s at 16kHz; increase if you want longer
target_layer = 8           # XLS-R hidden layer to pool (0..24 for 300M)

# --- load model/processor ---
processor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/wav2vec2-xls-r-300m")
base_model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-xls-r-300m", output_hidden_states=True)
...
def extract_batch(arrays):
    ...
    outputs = model(**inputs)  # hidden_states: tuple(layer) of [B, T, 1024]
    pooled = outputs.hidden_states[target_layer].mean(dim=1)  # [B, 1024]
