#!/usr/bin/env python3
"""
Linear classifier on pre-extracted XLS-R 300M layer 8 features for 7-way Swiss German dialect classification.

- Reads pre-extracted features from xlsr_300m_layer8.py: {split}_features.npy, {split}_labels.npy
...
# ---------------------- dataset ----------------------
class XLSRFeatureDataset(Dataset):
    """Dataset for pre-extracted XLS-R features."""
    def __init__(self, features_path: str, labels_path: str):
        self.features = np.load(features_path).astype(np.float32)  # [N, 1024]
...
class XLSRClassifier(nn.Module):
    """Simple MLP classifier for XLS-R features."""
    def __init__(self, input_dim: int = 1024, n_classes: int = 7):
        super().__init__()
...
def train():
    ...
    feat_dir = str(REPO_ROOT / "data_preparation/feature_extraction/ohne_deutsch/saved_features_xlsr_300m_layer8")
    save_dir = str(REPO_ROOT / "models/7_xlsr_300m_layer8")
    ...
    # Model
    model = XLSRClassifier(input_dim=1024, n_classes=n_classes).to(device)
    ...
    best_path = os.path.join(save_dir, '7_xlsr_300m_layer8_best.pt')
