#!/usr/bin/env python3
"""
Binary phoneme n-gram Linear Classifier: German (label_id=7) vs Non-German (label_id 0–6)
----------------------------------------------------------------------------------------
...
- Uses SGDClassifier with logistic loss for binary classification
...
SAVE_DIR  = str(REPO_ROOT / "models/7_linear_phoneme_binary")

# Vectorizer & Linear Classifier params
NGRAM_LO  = 2
NGRAM_HI  = 4
MIN_DF    = 2
MAX_DF    = 1.0
...
def train_and_eval(df_tr: pd.DataFrame, df_va: pd.DataFrame, df_te: pd.DataFrame):
    ...
    dump(clf, os.path.join(SAVE_DIR, "linear_model.joblib"))
