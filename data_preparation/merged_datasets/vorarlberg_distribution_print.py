...
def main():
    ...
    # Find all show directories (directories that contain an 'original' subdirectory)
    show_dirs = []
    for item in all_items:
        item_path = os.path.join(AUDIO_ROOT, item)
        if os.path.isdir(item_path):
            original_path = os.path.join(item_path, 'original')
            if os.path.isdir(original_path):
                show_dirs.append(item)
    
    if not show_dirs:
        print(f"⚠️  No show directories with 'original' subdirectory found in {AUDIO_ROOT}")
        sys.exit(0)
    
    print("="*60)
    print("📊 Vorarlberg Audio Distribution (Total hours/episodes per show)")
    print("="*60)
    ...
    for idx, show in enumerate(sorted(show_dirs), 1):
        original_path = os.path.join(AUDIO_ROOT, show, 'original')
        
        try:
            files_in_dir = os.listdir(original_path)
